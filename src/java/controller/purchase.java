...
public class purchase extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Product> productList = cd.getCartItems(accountId);
        ...
        for (Product p : productList) {
            for (int i = 1; i <= p.getStock(); i++) {
                ...
                productKeys.add(KeyGenerator.generateKey());
            }
            phd.addEntry(accountId, p.getProductId(), LocalDateTime.now());           
        }
        
        ad.modifyBalance(accountId, -cd.getCartTotal(accountId));
        
        cd.emptyCart(accountId);
        
        ContentDelivery.sendProductKey(account.getEmail(), productNames, productKeys);
        
        request.getRequestDispatcher("postpurchase.jsp").forward(request, response);
    }
